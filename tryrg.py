...
pulse_duration = 100  # Duration of each pulse in samples
...
plt.ylabel('Value')
plt.xticks(np.arange(0, len(data) * 100, 100))
plt.grid(True)

# Plot Manchester encoded signal
plt.subplot(3, 1, 2)
plt.plot(encoded_signal)
plt.title('Manchester Encoded Signal (Rectangular Pulses)')
plt.xlabel('Time (x 100)')
plt.ylabel('Amplitude')
plt.xticks(np.arange(0, len(encoded_signal) * 100, 100))
plt.grid(True)
